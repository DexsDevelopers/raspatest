...
if (isset($_POST['aprovar_saque'])) {
    ...
    try {
        ...
        if (!in_array($activeGateway, ['pixup', 'digitopay', 'lunarpay'])) {
            throw new Exception('Gateway "' . $activeGateway . '" não suportado para saques.');
        }
        ...
        } elseif ($activeGateway === 'lunarpay') {
            // LunarPay não possui API de saque — aprovação manual
            $stmt = $pdo->prepare("
                UPDATE saques
                SET status = 'PAID', gateway = 'lunarpay', updated_at = NOW()
                WHERE id = ?
            ");
            $stmt->execute([$saque_id]);

            $pdo->commit();
            $_SESSION['success'] = 'Saque marcado como PAGO manualmente (LunarPay). Lembre-se de transferir o PIX manualmente para o usuário.';
